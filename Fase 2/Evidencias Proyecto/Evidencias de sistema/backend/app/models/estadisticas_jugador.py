...
class EstadisticasJugador(Base):
    ...
    rut_jugador: Mapped[str] = mapped_column(
        String(10), ForeignKey("FICHA_JUGADOR.rut_jugador"), primary_key=True
    )
    id_serie: Mapped[int] = mapped_column(
        Integer, ForeignKey("SERIE.id_serie"), primary_key=True
    )
    ...
    ficha_jugador: Mapped["FichaJugador"] = relationship(
        "FichaJugador", back_populates="estadisticas_jugador"
    )
    serie: Mapped["Serie"] = relationship(
        "Serie", back_populates="estadisticas_jugador"
    )
