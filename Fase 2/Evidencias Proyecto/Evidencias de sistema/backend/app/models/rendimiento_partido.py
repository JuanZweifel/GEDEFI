...
class RendimientoPartido(Base):
    ...
    id_partido:Mapped[int] = mapped_column(Integer, ForeignKey("PARTIDO.id_partido"), primary_key=True, nullable=False, index=True)
    fecha_ini: Mapped[date] = mapped_column(Date, nullable=False)
    rut_jugador: Mapped[str] = mapped_column(String(10), ForeignKey("JUGADOR.rut_jugador"), nullable=False)
