...
class FichaJugador(Base):
    ...
    nacionalidad: Mapped[int] = mapped_column(
        Integer, ForeignKey("PAIS.id_pais"), nullable=False
    )  # FK a PAIS
    correo_electronico: Mapped[str] = mapped_column(
        String(320), nullable=False, unique=True
    )
    pierna_habil: Mapped[int] = mapped_column(
        Integer, nullable=True
    )  # 1: derecha, 2: izquierda, 3: ambas
    genero: Mapped[bool] = mapped_column(
        Boolean, nullable=False
    )  # True: masculino, False: femenino

    # Relaciones
    estadisticas_jugador: Mapped[list["EstadisticasJugador"]] = relationship(
        "EstadisticasJugador", back_populates="ficha_jugador"
    )

    evaluaciones_fisicas: Mapped[list["EvaluacionFisica"]] = relationship(
        "EvaluacionFisica", back_populates="ficha_jugador"
    )

    pais: Mapped["Pais"] = relationship("Pais", back_populates="jugadores")
