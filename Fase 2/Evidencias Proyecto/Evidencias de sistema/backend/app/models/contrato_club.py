...
class ContratoClub(Base):
    ...
    # Relaciones
    asociacion: Mapped[int] = relationship(
        Asociacion, back_populates="contratos_club", cascade="all, delete-orphan"
    )
    ...
    series: Mapped[list["Serie"]] = relationship(
        Serie, back_populates="contrato_club", cascade="all, delete-orphan"
    )
