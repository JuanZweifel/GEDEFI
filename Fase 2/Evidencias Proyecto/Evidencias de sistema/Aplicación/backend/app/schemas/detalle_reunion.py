...
class DetalleReunionCreate(ReunionBase):
    rut_usuario: str = Field(..., description="ID del usuario")
...
class DetalleReunionList(BaseModel):
    detalles_reunion: List[DetalleReunionRead] = Field(
        default_factory=list, description="Lista de asistencias de reuniones"
    )
