...
export const ClubModule: React.FC = () => {
    ...
    const filteredClubs = (() => {
        ...
        if (selectedEstado === "1") {
            baseList = clubList.filter((club) => club.club_activo === true);
        } else if (selectedEstado === "2") {
            baseList = clubList.filter((club) => club.club_activo === false);
        }

        // 2️⃣ Luego filtramos por término de búsqueda (input)
        if (!searchTerm.trim()) return baseList; // sin filtro adicional

        // Si el término son solo números, filtra por RUT
        if (/^\d+$/.test(searchTerm)) {
            return baseList.filter((club) =>
                club.rut_club?.includes(searchTerm)
            );
        }

        // Si contiene letras: buscar por nombre y por email
        if (/[a-zA-ZáéíóúÁÉÍÓÚ]/.test(searchTerm)) {
            const term = searchTerm.toLowerCase();

            const byName = baseList.filter((club) =>
                club.nombre_club.toLowerCase().includes(term)
            );

            const byEmail = baseList.filter(
                (club) =>
                    !byName.includes(club) && // evita duplicados
                    club.email_club.toLowerCase().includes(term)
            );
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <Input
                                        ...
                                        className="w-64"
                                    />
                                    <Select value={selectedEstado} onValueChange={(value: string) => setSelectedEstado(value)}>
